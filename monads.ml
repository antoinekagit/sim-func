...
module StateAbs_Accs : sig
   include MONAD with type 'a extract = ('a * Accs.t)
   type 'a monad_base = ('a monad)
   val run : 'a monad -> (Accs.t -> ('a * Accs.t))
   val get_accs : unit -> Accs.t monad
   val get_amount : ident -> amount monad
   val exc_make_transfert : Accs.transfert_details -> unit monad
                           
   module T : functor (M : MONAD) -> sig
                 include MONAD_TRANSFORMED
                         with type 'a monad_int = ('a monad_base)
                          and type 'a monad_ext = ('a M.monad)
                          and type 'a extract = ('a * Accs.t) M.extract
                 val run : 'a monad -> (Accs.t -> ('a * Accs.t) M.monad)
                 val get_accs : unit -> Accs.t monad
                 val get_amount : ident -> amount monad
                 val exc_make_transfert : Accs.transfert_details -> unit monad
              end
   ...
   type 'a monad_base = ('a monad)
   let run ma = ma
   let get_accs = get_state
   let get_amount ident accs = (Accs.get_amount ident accs, accs)
   let exc_make_transfert details accs =
     ((), Accs.exc_make_transfert details accs)

   module T (M : MONAD)  = struct
      include StateT(Accs)(M)
      let run ma = ma
      let get_accs = get_state
      let get_amount ident accs = M.return (Accs.get_amount ident accs, accs)
      let exc_make_transfert details accs =
        (M.return ((), Accs.exc_make_transfert details accs))
   end              
end 
